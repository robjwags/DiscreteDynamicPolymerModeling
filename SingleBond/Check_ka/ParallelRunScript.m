...
% BeadSpringOrMeso = 0;               %0 for bead-spring model, 1 for mesoscale
...
%% Input Parameters
Np = 20^3;       % Number of polymer pairs(make a cubic number) %DO NOT CHANGE
...
ea = logspace(-2,1,7)*kbT;  % Normalized bond activation energy for association in 
                    %units of [kbT]
...
Distances = 1/8;    % Separation between tether sites in units of Nb
...
if CompileEnsembleData==1
    CompileData(Package,OverrideCompile,OverrideCompute,ToggleDynamics,...
        ...
        CalculateBondKinetics,CalculateAlignmentt,CurrentFolder,OutputFolder,...
        NoProcessors);
end
...
if PostProcess==1
    PostProcessData(Package,OverridePostProcess,ToggleDynamics,...
        LengthConversion,DamperConversion,FitPowerOrExponent);
end
