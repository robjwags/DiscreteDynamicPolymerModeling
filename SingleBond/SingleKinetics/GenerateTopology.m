...
function GenerateTopology(Package,OverrideTop,OverrideIn,...
    CF,OF,TD,UnitType,LC,DC,BSOM,DTF,A_tab)
...
t0 = (b*LengthConversion)^2/DiffCoeff;
eastar = round(-log(ka*t0),2);
...
if ~isfile(InputFileName) || OverrideIn==1
    ...
    PrintInputFiles(N_Kuhn_out,b_out,ka_out,kd_out,f0_out,T_out,...
        dt_out,damp,t0,A,CurrentFolder);
end
...
global dims N Positions ConnectionsSP...
    ...
    StationaryStableNodes StationaryDynamicNodes DiffusingStableNodes...
    DiffusingDynamicNodes Lx Ly Lz

%% ONLY WORKS IF NS=1. SET UP A LOOP OVER ADD RANGE IF WANT TO MAKE IT GENERAL

N_mers = N/2*N_Kuhn+N/2;
...
ct = 0;
% ONLY POLYMERIZE ONE NODE INTO A CHAIN - LEAVE THE OTHER A DYNAMIC BONDING
% SITE
...
StationaryStableNodes = (1:N/2)';
StationaryDynamicNodes = (N/2+1:N)';

DiffusingDynamicNodes = zeros(N/2,1);
DiffusingStableNodes = zeros((N_Kuhn-2)*N/2,1);
dynct = 0; stabct = 0;
for node=1:length(StationaryStableNodes)
    TetherNode = StationaryStableNodes(node);
    for Segment=1:N_Kuhn+1
        ct = ct+1;
        NewNode = AddRange(ct);
        theta = 2*pi*rand(1);
        varphi = 2*pi*rand(1);
...
if CheckFig==1
    figure(1)
    hold on
    s=scatter3(Positions(StationaryStableNodes,1),Positions(StationaryStableNodes,2),...
        Positions(StationaryStableNodes,3),'b','filled');
    s.SizeData = 6;
    s=scatter3(Positions(StationaryDynamicNodes,1),Positions(StationaryDynamicNodes,2),...
        Positions(StationaryDynamicNodes,3),'r','filled');
    s.SizeData = 6;
    s=scatter3(Positions(DiffusingStableNodes,1),Positions(DiffusingStableNodes,2),...
        Positions(DiffusingStableNodes,3),'c','filled');
    s.SizeData=6;
    s=scatter3(Positions(DiffusingDynamicNodes,1),Positions(DiffusingDynamicNodes,2),...
...
for node=1:size(X_SP,1)
    for j=1:size(X_SP,2)
        if ConnectionsSP(node,j)~=0
            X_SP(node,j) = Positions(ConnectionsSP(node,j),1);
            Y_SP(node,j) = Positions(ConnectionsSP(node,j),2);
            Z_SP(node,j) = Positions(ConnectionsSP(node,j),3);
        else
            X_SP(node,j) = NaN;
            Y_SP(node,j) = NaN;
            Z_SP(node,j) = NaN;
...
for node=1:MaxConn
    Rx_SP(:,node) = -X_SP(:,node)+X_SP(:,end);
    Ry_SP(:,node) = -Y_SP(:,node)+Y_SP(:,end);
    Rz_SP(:,node) = -Z_SP(:,node)+Z_SP(:,end);
end
X_SP(:,end) = [];
Y_SP(:,end) = [];
Z_SP(:,end) = [];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function PrintInputFiles(N,b,ka0,kd0,f0,T,dt,damp,tau0,A,CurrentFolder)

global TopologyFileName OutputAtom_loc OutputBond_loc InputFileName...
    TurnOnDynamics SIOrNormalized BeadSpringOrMeso dtFact
...
if BeadSpringOrMeso==2
    ka0 = A/tau0*(3/2/pi/N)^(3/2)*exp(-ea);
    l0 = (2*N*b^2/3)^(0.5);%(3/2/N/b^2)^(-1/2);
end
% Define lengthscales
Nb = N*b;           %chain length
Nb2 = Nb*b;         %N*b^2
...
L = ['\nvariable    b2      equal ',num2str(b^2,'%.2e')]; Assembled = [Assembled,L];
...
tauka = 1/ka0;
tau_heal = 1/(kd0+ka0);
theal = 0.5e3*dt*dtFact;%0.5e3*dt*dtFact;
tload = 0;
trelax = 0;
nheal = ceil(theal/dt);
nload = ceil(tload/dt);
...
% N_dyn = 1;
tdata = dt*N_dyn;%dtFact*1/20;  %Basically measures with frequency matching 1x the diffusion timescale
ttot = nmax*dt;
NData = ceil(ttot/tdata);
iout = ceil(nmax/NData); 
ithermo = ceil(iout/2);
% N_dyn = ceil(t_dyn/dt);
% N_dyn = 1;%iout; %check dynamics every N_dyn steps 
L = '\n####################################'; Assembled = [Assembled,L];
...
elseif BeadSpringOrMeso==2
%     L = ['\nneighbor    ',num2str(N*b,'%.2e'),' bin']; Assembled = [Assembled,L];
%     L = ['\ncomm_modify cutoff  ',num2str(N*b,'%.2e')]; Assembled = [Assembled,L];
...
if BeadSpringOrMeso==0 % Bead-spring
    L = '\npair_coeff  4 4	${eps_b}  ${sig_b}  ${lambda_b}'; Assembled = [Assembled,L];
end
L = '\n'; Assembled = [Assembled,L];
...
if BeadSpringOrMeso==0 % Bead-spring
    ...
    L = '\ngroup	stationarydynamic type 2'; Assembled = [Assembled,L];
    L = '\ngroup	diffusingdynamic type 3'; Assembled = [Assembled,L];
    L = '\ngroup	diffusingstable type 4'; Assembled = [Assembled,L];
    L = '\ngroup	stickers type 2 3'; Assembled = [Assembled,L];
    L = '\n'; Assembled = [Assembled,L];
...
if TurnOnDynamics==1
    if BeadSpringOrMeso==0 % Bead-spring
        L = ['\nfix		dynamic stickers bond/dynamic ',num2str(N_dyn,'%.0f'),...
            ' 2 2 ${ka} ${kd} ${Ldyn} maxbond 1 jtype 3'];% bell ${fsens}'];
        Assembled = [Assembled,L];  
    elseif BeadSpringOrMeso==1 % Mesoscale
        L = ['\nfix		dynamic stickers bond/dynamic ',num2str(N_dyn,'%.0f'),...
            ' 2 3 ${ka} ${kd} ${Ldyn} maxbond 1 jtype 3'];% bell ${fsens}'];
        Assembled = [Assembled,L];
    elseif BeadSpringOrMeso==2 % Scaling Theory - can connect span of chain length
        L = ['\nfix		dynamic stationarydynamic bond/dynamic ',num2str(N_dyn,'%.0f'),...
            ' 1 1 ${ka} ${kd} ${cut_dyn} maxbond 1 gauss ${l0} ${ka0}'];
%         L = ['\nfix		dynamic stationarydynamic bond/dynamic ',num2str(N_dyn,'%.0f'),...
%             ' 1 1 ${ka} ${kd} ${cut_dyn} maxbond 1 rouse ${b2}'];
% %         L = ['\nfix		dynamic stationarydynamic bond/dynamic ',num2str(N_dyn,'%.0f'),...
% %             ' 1 1 ${ka} ${kd} ${cut_dyn} maxbond 1'];
        Assembled = [Assembled,L];    
    end
end
...
global FileTag Positions dims Lx Ly Lz Np ConnectionsSP...
    ...
    StationaryDynamicNodes StationaryStableNodes DiffusingStableNodes...
    DiffusingDynamicNodes
...
Assembled = [];
if BeadSpringOrMeso==0 % Bead-spring
    N_atom_types = 4;   % Stationary Kuhn bead, diffusing Kuhn bead, diffusing dynamic, stationary dynamic
...
elseif BeadSpringOrMeso==2 % Theory
    N_atom_types = 1;   % Stationary node - counts as the tether and sticker location
    N_bond_types = 1;   % Dynamically bonded chains
end
...
for i=1:N_bond_types
    ...
    if BeadSpringOrMeso==0 % Bead-spring
        if type==1
            coeff1 = 1.1*3*kb*T/b^2; % make as stiff as possible to conserve Kuhn length
            coeff2 = 0.8*b;
        elseif type==2
    ...
    elseif BeadSpringOrMeso==2 % Mesoscale
        coeff1 = 3/2*kb*T/(N_Kuhn*b^2);
        coeff2 = 0;
    end
    L = ['\n',num2str(type),' ',...
...
if BeadSpringOrMeso==0 % Bead-spring
    for i=1:N_atoms
        ...
        if ismember(i,StationaryStableNodes)
            atom_type = 1;
        elseif ismember(i,StationaryDynamicNodes)
            atom_type = 2;
...
elseif BeadSpringOrMeso==1 % Mesoscale
    ...
    for i=1:N_atoms
        ...
        elseif ismember(i,StationaryDynamicNodes)
            atom_type = 2; %Stationary dynamic bonds
        elseif ismember(i,DiffusingDynamicNodes)
            atom_type = 3; %Diffusing dynamic bonds                                 
        end
...
global dims N Positions Ns ConnectionsSP...
    X_SP Y_SP Z_SP Rx_SP Ry_SP Rz_SP N_Kuhn b...
    StationaryStableNodes StationaryDynamicNodes DiffusingDynamicNodes...
    Lx Ly Lz
...
if CheckFig==1
    ...
    s.SizeData = 6;
    s=scatter3(Positions(StationaryDynamicNodes,1),...
        Positions(StationaryDynamicNodes,2),...
        Positions(StationaryDynamicNodes,3),'r','filled');
    s.SizeData = 6;
...
% ONLY WORKS IF NS=1
ConnectionsSP = zeros(N+1/2*Ns*N,Ns+1);
ConnectionsSP(:,end) = (1:size(ConnectionsSP,1))';
