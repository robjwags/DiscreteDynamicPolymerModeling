...
% Running parameters
% NData = 500;  
tdata = dt*dtFact*1/2;  %Basically measures with frequency matching 1/2x the diffusion timescale
ttot = nmax*dt;
NData = ceil(ttot/tdata);
iout = ceil(nmax/NData); 
ithermo = ceil(iout/2);
% N_dyn = ceil(t_dyn/dt);
N_dyn = 2;%iout; %check dynamics every N_dyn steps 
L = '\n####################################'; Assembled = [Assembled,L];
