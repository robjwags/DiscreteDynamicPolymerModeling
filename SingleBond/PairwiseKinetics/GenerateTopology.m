...
if BeadSpringOrMeso==2
    ka0 = A/tau0*(3/4/pi/N)^(3/2)*exp(-ea);
    l0 = (2*(2*N)*b^2/3)^(0.5);%(3/2/N/b^2)^(-1/2);
end
% Define lengthscales
Nb = N*b;           %chain length
Nb2 = Nb*b;         %N*b^2
...
% NData = 500;  
tdata = dt*N_dyn; %Basically measures with frequency matching 1x the diffusion timescale
ttot = nmax*dt;
...
if BeadSpringOrMeso==1 % Mesoscale
    ...
    L = '\nfix      free stickers brownian ${T} 1234 gamma_t ${damp}'; Assembled = [Assembled,L];
...
if TurnOnDynamics==1
    ...
    elseif BeadSpringOrMeso==0 % Bead-spring
        L = ['\nfix		dynamic endgroups bond/dynamic ',num2str(N_dyn,'%.0f'),...
            ' 3 3 ${ka} ${kd} ${Ldyn} maxbond 1'];% bell ${fsens}'];
        Assembled = [Assembled,L];    
    elseif BeadSpringOrMeso==2 % Bead-spring
...
elseif BeadSpringOrMeso==0% Bead-spring
    ...
    N_bond_types = 3;   % Kuhn segments and dynamic bonds
...
if BeadSpringOrMeso==0 || BeadSpringOrMeso==1
    ...
    L = '\n'; Assembled = [Assembled,L];
    if BeadSpringOrMeso==0 % Mesoscale
