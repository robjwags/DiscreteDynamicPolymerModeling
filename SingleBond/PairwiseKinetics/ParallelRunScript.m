...
OverrideRun = 1;                    %1 to rerun simulations
OverrideCompile = 1;                %1 to override compilation of sim data
OverrideCompute = 1;                %1 to override computation of stress, end-to-end vecs, MSDs, etc.
OverridePostProcess = 1;            %1 to override generation of stress plots
...
%% Input Parameters
ToggleDynamics = 1;                 %1 for dynamics 0 for control systems (no dynamics)
...
ea = 0.01*kbT;%logspace(-2,0,2)*kbT;  % Normalized bond activation energy for association in 
                    %units of [kbT]
...
Distances = linspace(0.0125,0.5,8);        % Separation between tether sites in units of Nb
...
if PostProcess==1
    PostProcessData(Package,OverridePostProcess,ToggleDynamics,...
        LengthConversion,DamperConversion,BeadSpringOrMeso,...
        FitPowerOrExponent,AppendScalingData);
end
