...
for i=1:length(ka_in)
    ...
    SetDirAndFileNames;
    DefineAssembledFileNames(ka);

%     %% Assemble the Ensemble Data
%     AssembleEnsembleDynamicsData(OverridePostProcess,Package,damps,N_Kuhns,Np,Ns,T,Nb,...
...
PlotScalingTheoryPhaseSpace(OverridePostProcess,ka_in,N_Kuhns,b,DiffCoeffs,...
    Package)
close all

%% Plot outputs swept over activation energy
PlotOutputsOverActivationEnergy(N_Kuhns,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function PlotScalingTheoryPhaseSpace(OverridePostProcess,ka_in,N_Kuhns,b,D,...
    Package)
...
Separations = unique(Package(:,14));
xplot = (linspace(0,Separations(end),100))';

for ea_i = 1:length(eaStar)
    ea = eaStar(ea_i);

    figure(ea_i); clf; hold on
    color_rng = (linspace(0,1,length(N_Kuhns)))';
    colors = [zeros(size(color_rng)) flipud(color_rng) zeros(size(color_rng))];
    for nk_i = 1:length(N_Kuhns)
        N_Kuhn = N_Kuhns(nk_i);

        sigma_a_theor = (3/2)^(-1/2)*sqrt(N_Kuhn)*b;
        k_encounter = ((3/2/pi)^(3/2))*D/(NormLength^2)*N_Kuhn^(-3/2);
        ka0_theor = k_encounter*exp(-ea);
        yplot = ka0_theor*exp(-(xplot.^2)/(sigma_a_theor^2));

        p(nk_i) = plot(xplot/(sqrt(N_Kuhn)*b),yplot*tau0);
        p(nk_i).LineStyle = '-';
        p(nk_i).Color = colors(nk_i,:);
        p(nk_i).LineWidth = 1;
    end
    ...
    saveas(gcf,['Output Plots/Scaling Theory/ka_theor versus lambda-ea.',num2str(eaStar),'.png'])
    saveas(gcf,['Output Plots/Scaling Theory/ka_theor versus lambda-ea.',num2str(eaStar),'.fig'])
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function PlotOutputsOverActivationEnergy(N_Kuhns,...
    A_bead,dA_bead,R2_bead,A_meso,dA_meso,R2_meso,R2_meso_vs_bead)

global FitPowerOrExponent FontSize OutputFolder
...
set(gcf,'Color','w')
pbaspect([1 1.25 1])
set(gcf,'Position',[100 100 310 310])
...
if ~isfile(FittedDynamicsFileName) || Override
    ...
    %% ADD OUTPUTS FOR KA1
    ka0_bead = zeros(size(N_Kuhns));
    sig_eff_bead = zeros(size(N_Kuhns));
    R2_bead = zeros(size(N_Kuhns));
    ka0_meso = zeros(size(N_Kuhns));
    sig_eff_meso = zeros(size(N_Kuhns));
    ...
    for nk = 1:length(N_Kuhns)
        ...
        if NormalizeRates==1
            ka_meso_temp = ka_meso(:,nk)/timenorm*1e3;
            ka_bead_temp = ka_bead(:,nk)/timenorm*1e3;
            kaSE_meso_temp = kaSE_meso(:,nk)/timenorm*1e3;
            kaSE_bead_temp = kaSE_bead(:,nk)/timenorm*1e3;

            kd_meso_temp = kd_meso(:,nk)/timenorm*1e3;
            kd_bead_temp = kd_bead(:,nk)/timenorm*1e3;
            kdSE_meso_temp = kdSE_meso(:,nk)/timenorm*1e3;
            kdSE_bead_temp = kdSE_bead(:,nk)/timenorm*1e3;
            if AppendScalingData
...
%         x = Separations;
%         y = ka_bead_temp;
%         [ka0_bead(nk),sig_eff_bead(nk),~] = FitGaussianToAttachment(2*N_Kuhn,b,x,y);
%         y = ka_meso_temp;
%         [ka0_meso(nk),sig_eff_meso(nk),~] = FitGaussianToAttachment(2*N_Kuhn,b,x,y);
        ...
        [A_bead(nk),dA_bead(nk),R2_bead(nk)] =...
            ...
            2*N_Kuhn,b,eaOverkbT);%,sigma_a_theor);
        ...
        if ismember(N_Kuhn,PlotCases)
            plotct = plotct+1;
            LegendEntries{plotct} = ['$N$ = (2)',num2str(N_Kuhn)];

            figure(1)
            xplot = (linspace(Separations(1),Separations(end),100))';
%             if ~PlotOnlyScalingTheory
%                 P(plotct) = plot(xplot/denoms(nk),ka0_bead(nk)/timenorm*exp(-(xplot.^2)/((sig_eff_bead(nk))^2)));
%                 P(plotct).LineStyle = '-';
%                 P(plotct).Color = Colorsn(plotct,:);
%                 P(plotct).LineWidth = 1.5;
% 
%                 Pm = plot(xplot/denoms(nk),ka0_meso(nk)/timenorm*exp(-(xplot.^2)/((sig_eff_meso(nk))^2)));
%                 Pm.LineStyle = '--';
%                 Pm.Color = Colorsn(plotct,:);
%                 Pm.LineWidth = 1.5;
%             else
%                 
%             end
            ...
            if PlotScalingTheory
                N_temp = 2*N_Kuhn;
                lambda = xplot/sqrt(N_temp)/b;

                P_encounter_bead = A_bead(nk)*((3/2/pi/N_temp)^(3/2))*D_in/(NormLength^2);
                ...
                P_encounter_meso = A_meso(nk)*((3/2/pi/N_temp)^(3/2))*D_in/(NormLength^2);
...
%             if PlotScalingTheory
%                 k_encounter_bead = A_bead(nk)*((3/2/pi)^(3/2))*D_in/(NormLength^2)*N_Kuhn^(-3/2);
%                 ka0_theor_bead = k_encounter_bead*exp(-eaOverkbT);
%                 yplot_bead = ka0_theor_bead*exp(-(xplot.^2)/(sigma_a_theor^2));
% 
%                 Ptheor(plotct) = plot(xplot/denoms(nk),yplot_bead);
%                 Ptheor(plotct).LineStyle = '-';
%                 Ptheor(plotct).Color = Colorsn(plotct,:);
%                 Ptheor(plotct).LineWidth = 1;
% 
%                 k_encounter_meso = A_meso(nk)*((3/2/pi)^(3/2))*D_in/(NormLength^2)*N_Kuhn^(-3/2);
%                 ka0_theor_meso = k_encounter_meso*exp(-eaOverkbT);
%                 yplot_meso = ka0_theor_meso*exp(-(xplot.^2)/(sigma_a_theor^2));
% 
%                 Ptheor_meso(plotct) = plot(xplot/denoms(nk),yplot_meso);
%                 Ptheor_meso(plotct).LineStyle = '-.';
%                 Ptheor_meso(plotct).Color = Colorsn(plotct,:);
...
%             end
        ...
        e =  errorbar(Separations/denoms(nk),fa_meso_temp,faSE_meso_temp);
        e.LineStyle = 'none';
        e.Color = 'c';
        e.Marker = '^';
        e.MarkerFaceColor = 'none';
        ...
        e =  errorbar(Separations/denoms(nk),fd_meso_temp,fdSE_meso_temp);
        e.LineStyle = 'none';
        e.Color = 'r';
        e.Marker = '^';
        e.MarkerFaceColor = 'none';
...
for nk = 1:length(N_Kuhns)
    ...
    if ~isfile(SampName) || Override
        ...
        for sp = 1:length(Separations)
            ...
            figure(1)
            s1 = scatter(time_bead(rng,sp,nk),smoothka_bead(rng,sp,nk));
            s1.MarkerEdgeColor = Colorsa(sp,:);
            s1.Marker = 'o';
            s1.SizeData = DataSize;

            p1(sp) = plot(time_meso(:,sp,nk),smoothka_meso(:,sp,nk));
            ...
            if AppendScalingData
                ScatterSpacing = round((size(time_thry,1)-1)/Npt);
                rng_thry = (1:ScatterSpacing:size(time_thry,1));
                s1a = scatter(time_thry(rng_thry,sp,nk),smoothka_thry(rng_thry,sp,nk));
            ...
            figure(2)
            s2 = scatter(time_bead(rng,sp,nk),smoothkd_bead(rng,sp,nk));
            s2.MarkerEdgeColor = Colorsd(sp,:);
            s2.Marker = 'o';
            s2.SizeData = DataSize;

            p2(sp) = plot(time_meso(:,sp,nk),smoothkd_meso(:,sp,nk));
            p2(sp).Color = Colorsd(sp,:);
            p2(sp).LineWidth = 1.5;
            p2(sp).LineStyle = '-';

            if AppendScalingData
                s2a = scatter(time_thry(rng_thry,sp,nk),smoothkd_thry(rng_thry,sp,nk));
            ...
            figure(3)
            s3 = scatter(time_bead(rng,sp,nk),fa_bead(rng,sp,nk)*100);
            s3.MarkerEdgeColor = Colorsa(sp,:);
            s3.Marker = 'o';
            s3.SizeData = DataSize;

            p3 = plot(time_meso(:,sp,nk),fa_meso(:,sp,nk)*100);
            p3.Color = Colorsa(sp,:);
            p3.LineWidth = 1.5;
            p3.LineStyle = '-';

            if AppendScalingData
                s3a = scatter(time_thry(rng_thry,sp,nk),fa_thry(rng_thry,sp,nk)*100);
            ...
            figure(4)
            s4 = scatter(time_bead(rng,sp,nk),fd_bead(rng,sp,nk)*100);
            s4.MarkerEdgeColor = Colorsd(sp,:);
            s4.Marker = 'o';
            s4.SizeData = DataSize;

            p4 = plot(time_meso(:,sp,nk),fd_meso(:,sp,nk)*100);
            p4.Color = Colorsd(sp,:);
            p4.LineWidth = 1.5;
            p4.LineStyle = '-';

            if AppendScalingData
                s4a = scatter(time_thry(rng_thry,sp,nk),fd_thry(rng_thry,sp,nk)*100);
        ...
        for sp = 1:size(PackageTemp,1)
            ...
            if ~isempty(PackageTemp) && (~isfile(EnsembleDynamicsFileName) ||...
                ...
                if AppendScalingData
                    ...
                    BSOM = 2;
                    [time_thry,ka_thry,kd_thry,Na_thry,Nd_thry,fa_thry,fd_thry,...
...
if ~isfile(AllDynamicsFileName) || Override
    AllDynamics.time_bead = time_all_bead;
    ...
    AllDynamics.time_meso = time_all_meso;
    ...
    if AppendScalingData
        AllDynamics.time_thry = time_all_thry;
...
global ka_all_bead kd_all_bead...
    Na_all_bead Nd_all_bead...
    fa_all_bead fd_all_bead...
    tau_d1_all_bead tau_a_all_bead tau_d_all_bead...
    ka_all_meso kd_all_meso...
    ...
    fa_all_thry fd_all_thry...
    tau_d1_all_thry tau_a_all_thry tau_d_all_thry...
    time_all_bead time_all_meso time_all_thry...
    AppendScalingData

time_all_bead = zeros(size(time_bead,1),NoSep);
...
time_all_meso = zeros(size(time_meso,1),NoSep);
...
if AppendScalingData
    time_all_thry = zeros(size(time_thry,1),NoSep);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [time,ka,kd,Na,Nd,fa,fd,tau_d1,tau_a,tau_d,...
...
if find_conf_int==1
    ...
    ft = fittype(['A*(',num2str(coeff1/N_Kuhn),')^(3/2)*exp(-',num2str(eaOverkbT),'-3/2*x^2)']);
    options = fitoptions('Method', 'NonlinearLeastSquares','Lower',0.99*A,'Upper',1.01*A);
    func = fit(lambda,y,ft,options);

    figure(100); clf; hold on
    scatter(lambda,y)
    plot(lambda,func(lambda),'k--')

    A = coeffvalues(func);
    delta_A = confint(func);
    delta_A = diff(delta_A)/2;
else
    delta_A = 0;
end

RSS = sum((y-func(lambda)).^2);
TSS = sum((y-mean(y)).^2);
R2 = 1-RSS/TSS;
